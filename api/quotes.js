...
export default async function handler(req, res) {
    ...
    if (ret.error) {
        res
            .status(401)
            .json(formatErr(401, 'quotes', ret.error));
        return;
    }
    ...
    if (limit) {
        if (!checkNumeric(limit)) {
            res
                .status(400)
                .json(formatErr(401, 'quotes', 'Error limit not numeric.'));
            return;
        }
    }
    ...
    if (Db.connection() === null) {
        res
            .status(401)
            .json(formatErr(401, 'quotes', 'Error in DB Connection.'));
        return;
    }
